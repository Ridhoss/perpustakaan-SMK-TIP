...
@section('content')
    ...
    <div class="d-flex position-relative align-items-center justify-content-between p-4">
        <h1 class="h3 mb-0 text-gray-800 font-primary">Detail Informasi Peminjaman</h1>
    ...
    <form action="/pinjambukusiswa" method="POST">
        @csrf
        <div class="col-12 mt-4 rounded-3 p-4 d-block d-lg-flex flex-column shadow">
            ...
            <div class="container shadow mt-4 mt-lg-0 p-4 rounded mb-4">
                ...
                <h1 class="nopem text-secondary mb-4">Nomor Peminjaman : {{ $kode }}</h1>
                ...
                <div class="row mb-2 px-2">
                    ...
                    <input type="text" class="form-control text-center" name="anggota"
                        placeholder="{{ $anggota->nisn }} - {{ $anggota->name }}" readonly>
                    <input type="hidden" name="id_anggota" value="{{ $anggota->nisn }}">
                </div>
                <div class="row mb-2 px-2">
                    <label class="mb-2 fw-medium">Tanggal Peminjaman</label>
                    <input type="text" class="form-control text-center" name="pinjam"
                        placeholder="{{ \Carbon\Carbon::createFromFormat('Y-m-d', $tgl_pinjam)->format('d F Y') }}"
                        readonly>
                ...
                <div class="row mb-2 px-2">
                    ...
                    <input type="text" class="form-control text-center" name="kembali"
                        placeholder="{{ \Carbon\Carbon::createFromFormat('Y-m-d', $tgl_kembali)->format('d F Y') }}"
                        readonly>
                ...
                <div class="row mb-2 px-2">
                    ...
                    <input type="text" class="form-control text-center" name="petugas"
                        placeholder="{{ $petugas->name }}" readonly>
            ...
            <div class="container shadow mt-4 mt-lg-0 p-4 rounded">
                ...
                @foreach ($buku as $bukus)
                    <input type="hidden" name="buku[]" value="{{ $bukus->isbn }}">

                    <div class="card mb-3" style="max-width: 400px;">
                        <div class="row g-0">
                            <div class="col-md-4">
                                <img src="{{ Storage::url('public/buku/' . $bukus->photo) }}"
                                    class="img-fluid rounded-start">
                            </div>
                            <div class="col-md-8">
                                <div class="card-body">
                                    <h1 class="card-text fs-4 text-uppercase fw-bold text-secondary w-100">
                                        {{ Str::limit($bukus->judul, 12, '..') }}</h1>
                                    <p class="card-text text-secondary mt-2 border-top pt-2">
                                        <span class="fw-bold">ISBN :</span> {{ $bukus->isbn }}<br>
                                        <span class="fw-bold">Pengarang :</span> {{ $bukus->pengarang }} <br>
                                        <span class="fw-bold">Tahun Terbit :</span> {{ $bukus->tahun_terbit }}
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <a href="/petpeminjamansiswa" type="button" class="btn btn-secondary me-2">Batal</a>
